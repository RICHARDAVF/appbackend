...
class InventarioWithLote(generics.GenericAPIView):
    def get(self,request,*args,**kwargs):
        data = {}
        try:
            host = kwargs['host']
            ...
            data['error'] = f"ocurrio un error: {str(e)}"
        return Response(data)
    def save(self,codigo,lote,fecha):
        date = self.request.data
        ...
        try:
            ...
            params = (date['almacen'],str(datetime.now().month).zfill(2),datetime.now(),
                        codigo,date['operacion'],date['ubicacion'],date['cantidad'],date['observacion'],
                        lote,fecha,date['user']['cod'],user[0],'E',datetime.now(),f"{mom_d_int}")
            sql = f"""
                    INSERT INTO toma{datetime.now().year} (ALM_CODIGO,MOM_MES,MOM_FECHA,
                    ART_CODIGO,OPE_CODIGO,UBI_COD1,MOM_CANT,mom_glosa,art_codadi,
                    mom_lote,usuario,doc_cod1,mom_tipmov,fechausu, mom_d_int) 
                    VALUES({','.join('?' for i in params)})
                """
            data = Querys(self.kwargs).querys(sql,params,'post',1)
    ...
    def post(self,request,*args,**kwargs):
        ...
        try:
            ...
            if len(datos['codigo'])>longitud:
                try:
                    fecha = '-'.join(i for i in [i for i in reversed(lote_fecha[-10:].split('/'))])
                    fecha = datetime.strptime(fecha,'%Y-%m-%d')
                    lote = lote_fecha[:-10]
                except :
                    lote = lote_fecha
            data = self.save(result1[0].strip(),lote,fecha)
