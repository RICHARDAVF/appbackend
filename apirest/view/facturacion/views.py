...
class Facturacion(GenericAPIView):
    ...
    def post(self,request,*args,**kwargs):
        ...
        try:
            ...
            if datos['tipo_documento']==1 and self.serie[0]!='B':
            ...
            sql = f"""
                    INSERT INTO guic{self.date.year} (
                        MOV_COMPRO,mov_fecha,MOV_CODAUX,DOC_CODIGO,MOV_MONEDA,mov_t_c,usuario,FECHAUSU,
                            ...
                            tie_efesol,gui_exp001,ori_codigo,tie_totalp
                    )
        ...
        try:
            ...
            for item in items:
                ...
                sql = f"""
                            ...
                            ubi_cod,ubi_cod1,mom_cant,mom_valor,mom_moneda,mom_t_c,mom_punit,usuario,fechausu,doc_compro,art_afecto,ped_observ,mom_dscto1,gui_inclu,art_codadi,mom_lote)
                            VALUES ({','.join('?' for i in params)})
                        """
                s,_ = CAQ.request(self.credencial,sql,params,'post')
                ...
                sql = f""" INSERT INTO movm{self.date.year}(
                            ...
                            ubi_cod,ubi_cod1,mom_cant,mom_valor,mom_moneda,mom_t_c,mom_punit,usuario,fechausu,art_afecto,ped_observ,mom_dscto1,gui_inclu,mom_redoc2,mom_codaux,
                            doc_cod1,art_codadi,mom_lote)
                            VALUES({','.join('?' for i in params)})
                        """
                s,_ = CAQ.request(self.credencial,sql,params,'post')
