...
import base64
load_dotenv()
class GuiasView(generics.GenericAPIView):
            ...
            if dates is None :
                ...
                sql = f""" 
                    INSERT INTO 
                        ...
                        DEP_CODIGO,PRO_CODIGO,DIS_CODIGO,AUX_TIPOPE,AUX_DOCUM,AUX_EDI,aux_cuenta,aux_cuentd,aux_condic,
                        aux_estado,aux_tipdoc,aux_fecoru,aux_docadi,aux_email) 
    ...
    def query(self,sql,params,opt='get'):
        credentials = (
            os.getenv('BD_HOST_SMAR_FOOT'),
            os.getenv('BD_NAME_SMAR_FOOT'),
            os.getenv('BD_USER_SMAR_FOOT'),
            os.getenv('BD_PASS_SMAR_FOOT'),
        )
        conn = QuerysDb.conexion(*credentials)
        cursor = conn.cursor()
...
class PDFFACTView(generics.GenericAPIView):
    ...
    def query(self,sql,params,opt='get'):
        crendentialss = (os.getenv('BD_HOST_SMAR_FOOT'),
                         os.getenv('BD_NAME_SMAR_FOOT'),
                         os.getenv('BD_USER_SMAR_FOOT'),
                         os.getenv('BD_PASS_SMAR_FOOT'),
                         )
        conn = QuerysDb.conexion(*crendentialss)
        cursor = conn.cursor()
        cursor.execute(sql,params)
        data = ''
        if opt =='get':
